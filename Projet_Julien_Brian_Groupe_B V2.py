...
with open("EIVP_KM.csv","r") as mesures : # Ouverture du fichier csv
    ...
    while i != 7882 :
        if int(ligne[0]) == idd : # Tant que l'id du capteur est bon, on affecte aux listes les valeurs correspondantes 
            ...
            if idd == 6 and i == 7882 : # Condition finale pour le dernier capteur
                date_init = date[0]
                tps = []
                for k in range(len(date)) : # Convertion des dates en secondes en fonction de la première date
                    tps.append(distance_temporelle(date[k])/3600/24)
                plt.title('Relevé du capteur ' + str(idd) + ' : bruit, temprérature et humidité')
                plt.xlabel('Temps en jours')
                plt.ylabel('dB / °C / %') # Tout ça pour faire jolie
                plt.xlim(0,20174.15)
                plt.plot(tps,bruits)
                plt.plot(tps,temp)
                ...
                plt.xlabel('Temps en jours')
                plt.ylabel('Lux / PPM')
                plt.plot(tps,lum,color='cadetblue')
                plt.plot(tps,co2)
                plt.show()
