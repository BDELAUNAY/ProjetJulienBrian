...
nb_cap = int(input("Entrez le nombre de capteur à étudier (entre 1 et 6). Attention l'indice de covariance n'est disponible que pour deux capteurs : "))
if nb_cap == 2 :
    corr = "Voulez vous afficher l'indice de corrélation des données que vous allez choisir ? (Oui ou Non) "
    bol_corr = ( 'Oui' == corr )
...
if nb_donnee != 1 :
    tous = input("Pour finir, voulez vous afficher ces courbes dans des graphiques différents ? (Oui ou Non) ")
bol_tous = (tous == 'Oui')
...
def titre(liste_etude_cap, liste_etude_donnee): # [0,1,5],[1,3] (Pour Température, )
    return('Relevé capteur 1, 2, 6 : Température, Luminosité')
...
def graph1(id_donnee,bol_moy,bol_tous): # Prend en argument deux booléens pour avoir ou non la moyenne avec la donnée d'un ou plusieurs capteur/s 
    if bol_tous == False :
...
def graph2() :
    if bol_intervalle :
        ...
        plt.title(titre(liste_etude_cap,liste_etude_donnee))
        plt.xlabel('Temps en demi-journée')
        plt.ylabel(lab(liste_etude_donnee))
        plt.xlim(0,30)
        for id_donnee in liste_etude_donnee : 
            for idd in liste_etude_cap :
                plt.plot(liste_int[idd-1][-1][2],liste_int[idd-1][id_donnee][2], color = liste_color[0], label = liste_int[idd-1][id_donnee][0])
            if bol_tous :
                plt.show()
