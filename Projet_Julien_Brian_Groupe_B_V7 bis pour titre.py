...
# defintiion des focntions titre et label en y

def titre(liste_etude_cap, liste_etude_donnee): #[0,5][2,4]:
    titre = "relevé capteur "
    n = len(liste_etude_cap)-1
    count = 0
    for i in liste_etude_cap:
        ...
        count += 1
    count = 0
    n= len(liste_etude_donnee)-1
    for j in liste_etude_donnee:
        if count != n:
            titre = titre + str(liste_cap[liste_etude_cap[0]][j][0]) +", "
        else:
            titre = titre + str(liste_cap[liste_etude_cap[0]][j][0]) + " :"
        count +=1
    return titre

def lab(liste_etude_cap, liste_etude_donnee):
    lab = ""
    n = len(liste_etude_donnee)-1
    count = 0
    for j in liste_etude_donnee:
        if count != n:
            lab = lab + liste_cap[liste_etude_cap[0]][j][1] + ", "
        else:
            lab = lab + liste_cap[liste_etude_cap[0]][j][1]
        count +=1
    return lab 
